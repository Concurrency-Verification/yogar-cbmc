...
void smt2_convt::convert_expr(const exprt &expr)
{
	...
	{
		boolbv_widtht boolbvWidtht(ns);
		
		unsigned result_width=boolbvWidtht(expr.type());
		...
		unsigned op_width=boolbvWidtht(expr.op0().type());
...
void smt2_convt::convert_union(const union_exprt &expr)
{
	...
	unsigned total_width=boolbvWidtht(union_type);
	
	if(total_width==0)
		throw "failed to get union width for union";
	
	unsigned member_width=boolbvWidtht(op.type());
...
void smt2_convt::convert_with(const with_exprt &expr)
{
	...
	else if(expr_type.id()==ID_union)
	{
		...
		unsigned member_width=boolbvWidtht(value.type());
...
void smt2_convt::convert_type(const typet &type)
{
	...
	else if(type.id()==ID_vector)
	{
		...
		else
		{
			...
			unsigned width=boolbvWidtht(type);
